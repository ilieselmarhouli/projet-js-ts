...
import type { ChartType, PeriodOption, Stock } from "../models/stock.js";
import { clearElement, createElement } from "../utils/dom.js";

const PERIODS: Array<{ label: string; value: PeriodOption }> = [
...
export class App {
  ...
  private readonly stockChart = new StockChart();
  private stocks: Stock[] = [];
  private selectedSymbols: string[] = [];
  private period: PeriodOption = 30;
  private chartType: ChartType = "line";

  private errorBox!: HTMLDivElement;
  private stockGrid!: HTMLElement;
  private legend!: HTMLDivElement;
  private statsGrid!: HTMLElement;
  private loader!: HTMLDivElement;
  private emptyState!: HTMLDivElement;
  private canvas!: HTMLCanvasElement;
  private periodButtons = new Map<PeriodOption, HTMLButtonElement>();
  private chartTypeButtons = new Map<ChartType, HTMLButtonElement>();
  ...
  private renderLayout(): void {
    ...
    periodBlock.append(
      createElement("span", { className: "toolbar-label", textContent: "Periode" }),
      this.createButtonGroup(PERIODS, this.periodButtons, this.period)
    );

    const chartTypeBlock = createElement("div", { className: "toolbar-block" });
    chartTypeBlock.append(
      createElement("span", { className: "toolbar-label", textContent: "Type de graphique" }),
      this.createButtonGroup(CHART_TYPES, this.chartTypeButtons, this.chartType)
    );
    ...
    chartHeader.append(
      createElement("h2", { textContent: "Evolution des prix" }),
      (this.legend = createElement("div", { className: "legend" }))
    );
  ...
  private createButtonGroup<T extends string | number>(
  ...
  private updateButtonState<T>(registry: Map<T, HTMLButtonElement>, activeValue: T): void {
  ...
  private toggleSelection(symbol: string): void {
    if (this.stocks.length === 0) {
  ...
  private renderLegend(selectedStocks: Stock[]): void {
  ...
  private renderStats(selectedStocks: Stock[]): void {
  ...
  private toggleLoader(isVisible: boolean): void {
